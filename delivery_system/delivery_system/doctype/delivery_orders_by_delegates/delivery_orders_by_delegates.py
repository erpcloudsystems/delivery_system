...
class DeliveryOrdersByDelegates(Document):
	def validate(self):
	...
	def before_submit(self):
		setteled_amount = flt(self.advance_amount_adjusted + self.advance_paid + self.unpaid_amount)
		if flt(setteled_amount < self.total_after_tax):
			frappe.throw(_("Order Amount  = {0}<br> \
			Adjusted Amount = {1}<br> \
			Pending Amount to be adjust = {2}<br><br>\
			<b>You need to settle Pending Amount to be adjust in order to confirm order \
			</b>").format(abs(self.total_after_tax),abs(setteled_amount),abs(self.total_after_tax-setteled_amount)))

	def on_submit(self):
		frappe.db.set_value('Delivery Orders By Delegates', self.name, 'confirmed_by', frappe.session.user)
		frappe.db.commit()

		if self.delivery_status == "Completed":
			doBYdelegate(self)

		if self.delivery_status == "Rejected":
			sor = frappe.get_doc("Sales Order", self.sales_order)
			sor.cancel()
			frappe.msgprint("Processing, Order Review And Sales Order Cancelled")

			create_delegate_rejected_order(self)

		if self.delivery_status == "Delayed":
			create_delegate_delayed_order(self)	
...
def getSplitSetting(source):
	SplSet = frappe.db.sql(
		"""select IF(allow_split_payment = 1, 1,0) from `tabDelegate` where name = '{0}';""".format(
			source
		)
	)
	return SplSet
...
def doBYdelegate(doc):
	...
	make_delivery_note(doc.sales_order)
...
def create_delegate_rejected_order(doc):
	do = frappe.get_doc({
...
def create_delegate_delayed_order(doc):
	do = frappe.get_doc({
